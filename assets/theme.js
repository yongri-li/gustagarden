...
function getStickyHeaderOffset() {
  ...
  return (
    ...
    parseInt(
      ...
      parseInt(
        documentStyles.getPropertyValue('--enable-sticky-announcement-bar') || 0
      )
  );
}
...
var ScrollSpy = class extends HTMLElement {
  connectedCallback() {
    ...
    this.elementsToObserve = Array.from(this.querySelectorAll('a')).map(
      linkElement => document.querySelector(linkElement.getAttribute('href'))
    );
...
var AnnouncementBar = class extends CustomHTMLElement {
  ...
  _startPlayer() {
    // chnage it to only autplay on mobile
...
function getMediaSrcset(media, sizeList) {
  let srcset = [],
    supportedSizes =
      typeof media === 'string' ? sizeList : getSupportedSizes(media, sizeList);
    ...
    switch (name) {
      case 'hidden':
        if (!this.hidden) {
          ...
          this.querySelector('.prev-next-button--next')?.removeAttribute(
            'disabled'
          );
          this._staggerProductsApparition();
        } else {
...
var GalleryList = class extends HTMLElement {
  ...
  previous() {
    this.select(
      [...this.listItems].reverse().find(item => item.isOnLeftHalfPartOfScreen)
        .index
    );
  }
  next() {
...
var PressList = class extends CustomHTMLElement {
  ...
  previous() {
    this.select(
      (this.selectedIndex - 1 + this.pressItems.length) % this.pressItems.length
    );
  }
  next() {
    this.select(
          ...
          navItems.map((item, index) => {
            return new CustomKeyframeEffect(
              ...
              },
              {
                duration: 300,
                delay:
                  300 + 120 * index - Math.min(2 * index * index, 120 * index),
...
var ProductInventory = class extends HTMLElement {
  connectedCallback() {
    ...
    document
      ...
      ?.addEventListener('variant:changed', this._onVariantChanged.bind(this));
  }
  ...
  async _setupVisibility() {
    ...
    const animation = new CustomAnimation(
      new ParallelEffect([
        ...
        new ParallelEffect(
          Array.from(
            this.querySelectorAll('.product__thumbnail-item:not(.is-filtered)')
          ).map((item, index) => {
            return new CustomKeyframeEffect(
              item,
              {
...
var CartDrawer = class extends DrawerContent {
  ...
  async _rerenderCart(event) {
    ...
    if (
      ...
      const response = await fetch(
        `${window.themeVariables.routes.cartUrl}?section_id=${this.getAttribute(
          'section'
        )}`
      );
