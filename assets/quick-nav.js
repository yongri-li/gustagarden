if (!customElements.get('quick-nav')) {
  class QuickNav extends HTMLElement {
    constructor() {
      ...
      this.grandchildSelect = this.querySelector(
        'custom-select[data-level="3"]'
      );
    ...
    handleQuickNavChange(evt) {
      if (evt.target.matches('custom-select') && evt.detail.selectedValue) {
        const selectedOption = evt.target.querySelector(
          '[aria-selected="true"]'
        );
        ...
        if (evt.target.dataset.level === '1' && this.childSelect) {
          ...
          QuickNav.prepareSelect(
            this.grandchildSelect,
            evt.detail.selectedValue
          );
        }
      } else if (evt.target.closest('.price-range')) {
        const priceRangeMin = this.priceRange.querySelector(
          "[name='filter.v.price.gte']"
        ).value;
        const priceRangeMax = this.priceRange.querySelector(
    ...
    async parseCollectionInfo(url) {
      ...
      if (response.ok) {
        ...
        const productsContainer =
          tmpl.content.querySelector('[data-num-results]');
        if (productsContainer) {
          const numResults = parseInt(productsContainer.dataset.numResults, 10);
          if (numResults === 0) {
            this.submitButton.textContent = 'Keine Produkte';
            preventButtonEnable = true;
          } else if (numResults === 1) {
            this.submitButton.textContent = numResults;
          } else {
            this.submitButton.textContent = numResults;
          }
        } else {
        ...
        if (this.dataset.showPrice) {
          ...
          if (priceRangeElem) {
            ...
            const maxHeight =
              this.priceRange.querySelector('.price-range').clientHeight;
            this.priceRange.style.maxHeight = `${
              parseInt(maxHeight, 10) + 1
            }px`;
          } else {
            // eslint-disable-next-line
            console.warn(
